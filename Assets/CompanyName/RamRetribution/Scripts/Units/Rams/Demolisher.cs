            ...
            while (true)
            {
                ...
                foreach (var friens in results)
                {
                    if (friens.TryGetComponent(out IRam ram))
                    {
                        ...
                        //attackComponnent.GateDamage += 1; ќн доступен только дл€ чтени€. Ќадо его както мен€ть так чтобы теб€ не наругали
                ...
                {
                    //attackComponents[i].GateDamage -= 1; ќн доступен только дл€ чтени€.Ќадо его както мен€ть так чтобы теб€ не наругали
                }
            }
        }
