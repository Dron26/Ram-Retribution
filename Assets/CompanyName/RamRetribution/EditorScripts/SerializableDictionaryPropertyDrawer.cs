...
namespace CompanyName.RamRetribution.EditorScripts
{
	public class SerializableDictionaryPropertyDrawer : PropertyDrawer
	{
		...
		private static readonly GUIContent _sIconPlus = IconContent ("Toolbar Plus", "Add entry");
		private static readonly GUIContent _sIconMinus = IconContent ("Toolbar Minus", "Remove entry");
		private static readonly GUIContent _sWarningIconConflict = IconContent ("console.warnicon.sml", "Conflicting key, this entry will be lost");
		private static readonly GUIContent _sWarningIconOther = IconContent ("console.infoicon.sml", "Conflicting key");
		private static readonly GUIContent _sWarningIconNull = IconContent ("console.warnicon.sml", "Null key, this entry will be lost");
		private static readonly GUIStyle _sButtonStyle = GUIStyle.none;
		private static readonly GUIContent _sTempContent = new GUIContent();
		...
		private static readonly Dictionary<PropertyIdentity, ConflictState> _sConflictStateDict = new Dictionary<PropertyIdentity, ConflictState>();
		...
		public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
		{
			...
			var buttonWidth = _sButtonStyle.CalcSize(_sIconPlus).x;

			var labelPosition = position;
			labelPosition.height = EditorGUIUtility.singleLineHeight;
			if (property.isExpanded)
				labelPosition.xMax -= _sButtonStyle.CalcSize(_sIconPlus).x;
			...
			if (property.isExpanded)
			{
				...
				if(GUI.Button(buttonPosition, _sIconPlus, _sButtonStyle))
				{
				...
				foreach(var entry in EnumerateEntries(keyArrayProperty, valueArrayProperty))
				{
					...
					buttonPosition.height = EditorGUIUtility.singleLineHeight;
					if(GUI.Button(buttonPosition, _sIconMinus, _sButtonStyle))
					{
					...
					if(i == conflictState.ConflictIndex && conflictState.ConflictOtherIndex == -1)
					{
						...
						iconPosition.size =  _sButtonStyle.CalcSize(_sWarningIconNull);
						GUI.Label(iconPosition, _sWarningIconNull);
					}
					else if(i == conflictState.ConflictIndex)
					{
						var iconPosition = linePosition;
						iconPosition.size =  _sButtonStyle.CalcSize(_sWarningIconConflict);
						GUI.Label(iconPosition, _sWarningIconConflict);
					}
					else if(i == conflictState.ConflictOtherIndex)
					{
						var iconPosition = linePosition;
						iconPosition.size =  _sButtonStyle.CalcSize(_sWarningIconOther);
						GUI.Label(iconPosition, _sWarningIconOther);
					}
		...
		private static ConflictState GetConflictState(SerializedProperty property)
		{
			...
			if(!_sConflictStateDict.TryGetValue(propId, out conflictState))
			{
				conflictState = new ConflictState();
				_sConflictStateDict.Add(propId, conflictState);
			}
			return conflictState;
		}

		private static readonly Dictionary<SerializedPropertyType, PropertyInfo> _sSerializedPropertyValueAccessorsDict;
		...
		static SerializableDictionaryPropertyDrawer()
		{
			...
			_sSerializedPropertyValueAccessorsDict	= new Dictionary<SerializedPropertyType, PropertyInfo>();
			...
			foreach(var kvp in serializedPropertyValueAccessorsNameDict)
			{
				...
				_sSerializedPropertyValueAccessorsDict.Add(kvp.Key, propertyInfo);
			}
		}
		...
		{
			_sTempContent.text = text;
			return _sTempContent;
		}
		...
		public static object GetPropertyValue(SerializedProperty p)
		{
			...
			if(_sSerializedPropertyValueAccessorsDict.TryGetValue(p.propertyType, out propertyInfo))
			{
